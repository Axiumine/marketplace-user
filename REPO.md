# marketplace-user — reference

This file holds what `CLAUDE.md` used to carry: rationale, reference tables, troubleshooting and
history. `CLAUDE.md` keeps the identity, the routing table and the hard rules; read this file for the
detail behind any of them. See also [`README.md`](./README.md) for the app's strategy, every rejected
alternative and chunk numbers — that material is not duplicated here.

## Mutation gate: why it's hook-only, and how to reproduce a survivor
...
`ssr: false` is two routes, and the table above is the corrected one. It used to list `/login`,
...
Two registrations live here, writing two different collections. `/register` → `userRegister` →
...
carries both. No sign-in link on the seller page — `/login` here authenticates against `user` and
...
`/privacy` is the public half of code that lives in other repos, and none of it is here. It states
...
The reset credential lives in the URL fragment, and the confirm route is why. The mail sends
...
Never turn SSR on for an `/account` route. Rendering authenticated HTML on a server that sits
...
`/authenticated-*` (4026 / 4029) is the ShopOwner tier — pointing anything here at it fails closed.
...
- **`yarn start` runs `serve.mjs`, not the build output.** `vite build` emits `dist/server/server.js`,
  ...
  It is gated, and the import that makes that awkward is aliased in `vitest.config.ts`. `serve.mjs`
...
`src/features/map/pmtiles.ts` carries the same rule and does not look like it does. It is a
...
## Language — market choices, not names
