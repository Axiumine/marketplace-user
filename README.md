...
[![OpenSSF Scorecard](https://api.scorecard.dev/projects/github.com/Axiumine/marketplace-user/badge)](https://scorecard.dev/viewer/?uri=github.com/Axiumine/marketplace-user)
...
whenever a session cookie is present. Configs are in the parent workspace’s [`marketplace-nginx/`](https://github.com/Axiumine/marketplace-nginx) — see
...
- **`@axiumine/marketplace-common` is published to npm, and that is the only way an edit there reaches
  this app** — `3.0.0`, consumers on `^3.0.0` (ADR-037, ADR-047). ⚠️ Restated twice: this bullet said the
  package was *not* published, true until 2026-08-26, and then described a local deploy script that is
  **deleted** as of 2026-08-30. Nothing copies a build into this repo's `node_modules` any more, so
  `yarn install` is authoritative and can destroy nothing — it resolves the exact version this repo's
  `yarn.lock` names. An edit in common that this app needs is a published version and a range bump here.

---

## 8. Related documents

- [`marketplace-nginx/README.md`](https://github.com/Axiumine/marketplace-nginx/blob/main/README.md) — the whole platform edge: three vhosts, TLS, CSP, the HTML
...
- [`../CLAUDE.md`](https://github.com/Axiumine/fullstack-marketplace-blueprint/blob/main/CLAUDE.md) — the platform-wide conventions this app is bound by.
- [`../marketplace-shopowner/README.md`](https://github.com/Axiumine/marketplace-shopowner/blob/main/README.md) — the app this one borrows its client-side conventions from.
